...
public class NumberUtil {
	...
	/**
	 * 把double字符串格式成两位小数字符串
	 * @param d 需要转换的数字
	 * @return double 返回double两位小数格式字符串
	 */
    ...
    */  
	...
	public static Double divide(int scale,Number ... numbers){
		if (scale < 0||numbers.length<2) {
			...
			Integer length = numbers.length;
			if (null==numbers[0]||numbers[0].toString().equals("0.0")||numbers[0].toString().equals("0")||numbers[0].toString().equals("")){
				return 0.00;
			}
			BigDecimal def = new BigDecimal(numbers[0].toString());
			for (int i=1;i<length;i++){
